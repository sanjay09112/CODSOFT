...
static int numberofTasks, removedTaskCount = 0, completedStatus[10], taskIndex;
...
void addTaskToExisting() {
    if (removedTaskCount >= 1) {
        ...
        getline(cin, taskArray[numberofTasks + extraTasks - removedTaskCount - 1]);
    } else {
        cout << "Enter the new task to add" << endl;
        getline(cin, taskArray[(numberofTasks - 1) + extraTasks]);
    }
}

void displayTasks() {
    if (numberofTasks > 0) {
        for (int j = 0; j < numberofTasks + extraTasks - removedTaskCount; j++) {
...
void deleteTask() {
    if (numberofTasks > 0) {
        ...
        for (updateIndex = 0; updateIndex < numberofTasks + extraTasks - removedTaskCount; updateIndex++) {
